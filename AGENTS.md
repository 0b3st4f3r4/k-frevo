...
- **2026-09-24.** **O ordinal nu de capítulo sai do livro, e o portão passa a proibi-lo.** O §12 declara que o livro não imprime número de capítulo em prosa, e as duas renumerações deixaram **21 das 24 ocorrências** apontando para o capítulo errado: o corte, a janela de 252 e a promessa nascem no capítulo 3, e o capítulo 1 é *O número e a barra* (a palavra "corte" não aparece nele). Cada uma passa a nomear o objeto --- o corte, o alarme, o calendário ---, e "a barra do primeiro capítulo" (`12:81`, correta) entra por `\cref{prop:barra}`. O `conferir_ordinais` fica de fora para "capítulo anterior/seguinte", que são relativos e sobrevivem à renumeração.
