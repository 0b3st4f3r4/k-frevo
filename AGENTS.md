...
| o nível, o incremento, o retorno, o logaritmo, a banda do nível com a raiz do horizonte e o expoente do caminho com a sua dimensão | 2 |
| o quantil, o posto, a amostra, o viés e a reamostragem | 3 |
...
2026-09-25 — **o livro fica com um sumário só, o do começo.** O «Sumário completo» do fim saiu, e com ele a maquinaria que o alimentava: o bloco no `livro.tex`, o `sumario.tex` gerado, a entrada no `GERADOS` do `lab/executar.py` e a função que o escrevia. Ele existia porque um segundo `\tableofcontents` sai **vazio** (o LaTeX abre o .toc para escrita antes e o relê quando o comando aparece), e a cópia do .toc da compilação anterior resolvia isso. O que a medição mostrou é que a cópia era desnecessária: **o sumário do começo já carrega o livro inteiro** --- o .toc acumula as entradas do documento todo, Referências, listas e proposições incluídas --- e os números da lista conferem com a paginação real (Referências na folha 327, figuras na 343, tabelas na 365, proposições na 373). O mesmo mapa impresso duas vezes custava **doze páginas**: o livro desce de 388 para 376. O gerador ficou com um trabalho só --- os enunciados formais ---, e por isso mudou de nome: `lab/enunciados.py`, porque arquivo chamado sumário que não faz sumário é nome que mente.

2026-09-25 — **as bandas de margem entraram**, desenho escolhido pelo autor: a arte das duas capas em cinza (1 canal, chapa de preto) respira na margem externa --- infinito no verso, colapso no recto ---, discreta (22% de tinta) no miolo inteiro e firme (34%) nas seis folhas de parte, que saem do gancho `\@endpart` (a bandeira acende antes de a folha do título embarcar e a própria banda a consome no primeiro embarque). Fica a 3 mm da mancha e 3 mm do corte, longe da costura; a frente de fora não respira (capa, folha de rosto, epígrafe, sumário e abertura) e a contra-capa sai de cena com `\ClearShipoutPictureBG`. Custo medido em escala de livro: **+177 KB no PDF inteiro**, porque são quatro imagens embutidas e reusadas --- não é custo por página. Dois números que valem como método: **a 13% de tinta ninguém vê** a banda (o mínimo medido foi 222 num branco de 255, e a ponte de visão não a mencionou); e **o recorte da arte se escolhe por medição** --- o primeiro recorte do infinito caiu em branco puro (0,4% de tinta, porque a estrutura daquela arte vive entre x=366 e x=1647) e o do colapso, com 70% de cobertura, virava barra cinza sólida. A verificação saiu numa cópia de rascunho com os números e a figura que o ciclo E44 do autor ainda vai registrar: é esse ciclo que faz a árvore real compilar.
...
- **2026-09-25 — a fila de integração começa a ser cruzada em paralelo, e o limite é a infra, não o desenho.** Das 19 fontes da fila, três foram integradas e verificadas nesta rodada — `higuchi1988approach` no cap. 2 (o expoente do caminho e a dimensão que dele sai), `efron1979bootstrap` no cap. 3 (a barra que a amostra ergue sozinha; o gesto ganha endereço na raiz e os caps. 27 e 28 deixam de usar o que nunca foi construído) e `csiszar1975divergence` no cap. 11 (a projeção em divergência e a escolha canônica) — com dez projetistas e dez implementadores despachados em paralelo. A paralelização funcionou no desenho (dez desenhos completos, cada um lendo o capítulo e o módulo da sua travessia) e quebrou na implementação: a infra de agentes caiu no meio da onda, e o resto foi feito à mão. As outras seis pontes têm brief em `~/Ø/conserto/briefs/` e os módulos de quatro delas estão em `~/Ø/conserto/pendentes_modulos.patch`. Lição de método: paralelizar o DESENHO é barato e rende; paralelizar a ESCRITA precisa de infra que aguente a onda — e o desenho completo em disco é o que permitiu terminar à mão sem recomeçar.
...
- **2026-09-24.** **O contrato emagreceu por arquivamento, e não por poda --- a poda não pagava a conta.** O `AGENTS.md` estava em **73.329 bytes** contra o teto de ~67 KB declarado --- 70.870 era a conta em CARACTERES, que é como esta casa vinha medindo, e os acentos valem 2.459 bytes. A regra de poda do §12 tira a entrada cujo conteúdo já vive no cabeçalho do capítulo, no portão, no caderno ou no `.gitignore` que a criou; aplicada por medição --- frases de 45 caracteres ou mais com shingles de seis palavras já presentes nos 349 KB de `lab/` e `lib/frevolab/` ---, ela encontrou **uma** entrada em 71: a do ordinal nu, 659 bytes, cuja lição está inteira na docstring do `conferir_ordinais`, que passou a ser o registro dela. As outras 70 não têm duplicata em lugar nenhum --- são lição de método, que a mesma regra manda preservar. A saída foi então **outra operação, e não uma poda**: as 47 decisões do ciclo de 2026-09-23 desceram **na íntegra** para `AGENTS-archive.md` (sha256 conferido antes e depois), e ficaram no §12 as 24 do ciclo corrente mais o índice de uma linha por entrada arquivada. O contrato foi de **73.329 para 58.020 bytes** no arquivamento, e a lição continua achável por busca. E um número desta própria entrada mentiu duas vezes antes de ser conferido, cada vez de um jeito: as contagens que esta casa vinha chamando de KB eram de CARACTERES (70.870 caracteres contra 73.329 bytes --- 2.459 bytes só de acento), e o tamanho *com esta entrada dentro* não se imprime aqui, porque ele muda a cada edição da frase que o imprime. O teto se mede com `wc -c`, não com `wc -m`. E a regra passa a valer para todo ciclo que se fechar: ao fim do ciclo, as decisões dele descem para o arquivo na íntegra, e no §12 fica o índice.
- **2026-09-24.** **A barra que some deixa letra no papel, e nenhum portão vê.** A linha de baixo da tabela do capítulo 23 perdeu a barra numa correção antiga --- `umTetoPecasMaior` no lugar de `\numTetoPecasMaior` ---, e a célula passou a imprimir as letras no lugar do número. O portão dos dígitos não pega: ele procura dígito digitado à mão, e ali não há dígito nenhum. A varredura da classe é uma linha, e vale para todo comando: `grep -rnP '(?<!\\)\b(num|Teto)[A-Za-z]*\b'` em `livro/**/*.tex`. Ela achou **uma** ocorrência em vinte e quatro capítulos --- esta. A causa é conhecida desta casa: um `re.sub` em que a barra do comando entra na string de substituição e sai como escape.
