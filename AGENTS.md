...
- **2026-09-24.** **O motor do livro passou a ser declarado, e o portão passou a nomeá-lo.** O mesmo `latexmk` de sempre escolheu nesta árvore o LuaHBTeX em vez do pdfTeX, e o que ele escreve no log é `Missing character` para o apóstrofo tipográfico dos títulos da bibliografia: o livro compilava, o PDF saía e o defeito era uma letra a menos no papel. O portão da compilação acusou «log sem compilação` com um log de 46 KB dentro, porque procurava o pdfTeX e não dizia o que achou. Duas correções: o `-pdf` entrou no comando do §9.3, e a mensagem do portão passou a imprimir a máquina que escreveu o log.
