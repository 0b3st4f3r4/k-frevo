...
|---|---|---|---|
| **A raiz** | raiz | a medição, o corte e o preço de fixar | nada acima funciona sem isso: uma afirmação sem procedimento não é afirmação, e fixar finitamente é o que cria a possibilidade de errar. É também o fracasso mais palpável para o leitor |
...
- **2026-09-24.** **A mesma tolerância tinha duas unidades, e a prosa dizia uma enquanto o critério media na outra.** O capítulo 11 declara uma tolerância de 0,15, e ela vivia com dois significados: a proposição da meia-vida fala em fração do **degrau**, e o critério de viabilidade do caderno compara o erro relativo médio contra 0,15 do **nível novo**. Como o mundo do caderno dobra de uma vez, o degrau é metade do nível — de modo que 15% do nível é **30% do degrau**, e o mesmo número valia como duas coisas diferentes. Reproduzido com o `.venv/bin/python`, sem gravar nada: com 15% do nível, **3 de 48** pares (memória, horizonte) aprendem, que é o número impresso no livro; com 15% do degrau, **nenhum** dos 48 passa, porque o menor erro de toda a varredura é 0,1178. A decisão é do autor, e ele a tomou: **a tolerância é do nível novo**, que é a unidade do erro medido. O conserto separa as duas em todo o caminho — a biblioteca passou a ter `TOLERANCIA` (nível), `FATOR_PADRAO` e `TOLERANCIA_DEGRAU` (a mesma tolerância em degraus, 0,30); o `limiar` converte explicitamente em vez de usar um número só para as duas coisas; o caderno nomeia as duas constantes; e o capítulo declara a unidade nas quatro vezes em que cita o número. A coluna "dias previstos" da `tab:esquecimento` mudou por consequência: a conta do cruzamento até 15% do nível leva **24,69** dias para a memória de vinte e um dias (era 38,9, que era o cruzamento de 15% do degrau). E o fecho do capítulo passou a declarar o que antes insinuava falso: **24,69 é o instante em que o desvio cruza a tolerância, e 120 dias é o horizonte em que a média do erro fica dentro dela** — a média carrega os primeiros dias, quando o desvio ainda é o degrau inteiro, e comparar os dois como se fossem a mesma medida era o defeito. Três asserções novas no `auto_teste()` fixam a ponte (a conversão, o padrão dos dias e o caminho que a viabilidade percorre), porque a porta pela qual o defeito entrou foi **um número só com dois significados**.
...
- **2026-09-24.** **Três seções passaram a ter o que provar, e não só o que medir.** O §8.2 manda provar todo enunciado matemático ou marcá-lo como conjectura, e três capítulos afirmavam em prosa o que era demonstrável em menos de dez linhas: o **preço da partição** (cap. 4: com a janela repartida em $C$ células iguais o alarme mais raro é $C/(n+C)$ por dia, e com $C=1$ volta $1/(n+1)$), o **teto do par** (cap. 5: a chance de as duas pernas romperem juntas não passa de $p$ por monotonicidade, a independência previria $p^{2}$, e o par pode entregar até $1/p$ — vinte vezes no corte de 5%, que é a folga onde o 7,168 medido mora) e o **atraso que não cobra dias** (cap. 19: sob permutabilidade dos $n+1$ dias, o posto do dia de hoje é uniforme e a entrega é $k/(n+1)$ para todo $d$ — a conta do capítulo 1, agora com um atraso no meio). Nenhuma prova substitui medição: o que era medido continua medido.
...
- **2026-09-24.** **O medidor de clichê contava uma locução como clichê.** O padrão `\bde ponta\b` casava dentro de "de ponta a ponta" e acusava dois capítulos que não têm clichê nenhum. É a quarta vez que a régua erra (a antítese, o extrator de prosa, o log vazio) e a lição é a mesma: **antes de cortar por causa de uma contagem, conferir a contagem**. O padrão passou a exigir que a locução não venha colada, com prova nos dois sentidos: "uma solução de ponta" continua acusando, "de ponta a ponta" não acusa mais.

- **2026-09-24.** **A leitura integral do texto e das legendas, e a lição de onde o defeito morava.** Os vinte capítulos foram lidos de ponta a ponta (cinco leitores independentes nos capítulos 9 a 20) e as legendas de todas as figuras conferidas contra o código que as desenha, com cada comando cruzado contra o caderno. Vinte e uma correções, e quase todas no mesmo lugar: **a frase que comenta o número, não o número**. Três legendas afirmavam o que o próprio corpo do capítulo desmentia (o núcleo elíptico "o mesmo nos dois" num experimento construído justamente para abri-lo; "a janela que segue a mudança" atribuída às duas barras quando só uma é recortada; três séries anunciadas e duas nomeadas), e uma frase do corpo dava a um desenho o número de outro (o 32,5% medido na contenção de 80 dias apresentado como o experimento de 252). Junto: a contagem de capítulos que a divisão do capítulo 2 desatualizou ("dezoito" onde são dezenove), a promessa escrita em fração onde o livro a anuncia em porcentagem, "31 replicatas de 80 dias" para o que custa 100 (contenção mais espera), o exemplo mínimo que eu mesmo havia escrito com o antecedente errado ("anteriores a ela" no lugar de "anteriores à contenção"), "no meio do caminho" para uma mudança que cai a um quarto, dois rótulos de figura sem acento e dois travessões impressos como três hífens. E duas armadilhas de língua que atravessaram dez rodadas de portão: **"previria"** (do verbo previr) onde o livro quer **"preveria"** (de prever), em oito lugares, e **"1,168 vez"** onde numeral não unitário pede plural, em seis. A lição, na família das três anteriores: **o portão amarra número a medição, não afirmação a medição — e a legenda é a frase que ninguém relê.** A rodada seguinte virou a mesma leitura para as figuras: quatro auditores leram cada figura e cada tabela contra o desenho (o PNG pela ponte de visão, e a geometria medida no pixel), e acharam treze coisas --- três legendas contando mundos ou barras que o desenho não tem ("quatro mundos" com três mudanças e um controle, "à esquerda de cada barra" num gráfico de barras soltas, "a barra cheia" que nenhuma barra alcança), um painel rotulado "em razão do prometido" cujo alvo era a independência, a estrela do "melhor desenho" plotada na abscissa do pior, um eixo dizendo "blocos" para uma fração, um choque comum de sinal sorteado descrito como se fosse só de queda, e a coluna "piso de ruído" carregando o piso da mistura enquanto o texto citava o da janela. (O log foi podado nesta rodada: saíram os dez registros de capítulo escrito, cujo conteúdo vive no cabeçalho de cada capítulo, e sete entradas cuja lição vive no portão, no caderno, no comando ou no .gitignore que as criou.)
