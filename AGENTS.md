...
2026-09-25 — **o livro fica com um sumário só, o do começo.** O «Sumário completo» do fim saiu, e com ele a maquinaria que o alimentava: o bloco no `livro.tex`, o `sumario.tex` gerado, a entrada no `GERADOS` do `lab/executar.py` e a função que o escrevia. Ele existia porque um segundo `\tableofcontents` sai **vazio** (o LaTeX abre o .toc para escrita antes e o relê quando o comando aparece), e a cópia do .toc da compilação anterior resolvia isso. A cópia resolvia, e a medição mostrou que era desnecessária: **o sumário do começo já carrega o livro inteiro** (o .toc acumula Referências, listas e proposições) e os números da lista conferem com a paginação real. O mesmo mapa impresso duas vezes custava **doze páginas**: 388 para 376. O gerador ficou só com os enunciados e mudou de nome --- `lab/enunciados.py`, porque arquivo chamado sumário que não faz sumário é nome que mente.

2026-09-25 — **as bandas de margem entraram**, desenho escolhido pelo autor: a arte das duas capas em cinza (1 canal, chapa de preto) respira na margem externa --- infinito no verso, colapso no recto ---, discreta (22% de tinta) no miolo inteiro e firme (34%) nas seis folhas de parte, que saem do gancho `\@endpart` (a bandeira acende antes de a folha do título embarcar e a própria banda a consome no primeiro embarque). Fica a 3 mm da mancha e 3 mm do corte, longe da costura; a frente de fora não respira (capa, folha de rosto, epígrafe, sumário e abertura) e a contra-capa sai de cena com `\ClearShipoutPictureBG`. Custo medido em escala de livro: **+177 KB no PDF inteiro**, porque são quatro imagens embutidas e reusadas --- não é custo por página. Dois números de método: **a 13% de tinta ninguém vê** a banda (mínimo 222 num branco de 255, e a ponte de visão não a mencionou); e **o recorte da arte se escolhe por medição** --- o primeiro recorte do infinito caiu em branco puro (0,4% de tinta) e o do colapso, com 70% de cobertura, virava barra cinza sólida.
