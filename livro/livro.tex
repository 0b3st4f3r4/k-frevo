...
\input{capitulos/02_medida_corte_preco.tex}
\part{As voltas}
...
\input{capitulos/03_orcamento_do_alarme.tex}

% O capítulo 4 — volta 1: a forma da mudança decide o preço do silêncio.

\input{capitulos/04_forma_decide_o_preco.tex}

% O capítulo 5 — a segunda forma que a margem não vê: o calendário.

\input{capitulos/05_o_dia_da_semana.tex}

% O capítulo 6 — a última forma da volta 1: a dependência, que não está em margem nenhuma.

\input{capitulos/06_dia_em_que_tudo_cai}

% O capítulo 7 — a volta 2: a estatística que estima não anuncia, e o controle diz por quê.

\input{capitulos/07_estatistica_que_nao_anuncia}

% O capítulo 8 — a volta 3: quantas explicações cabem nos mesmos dados.

\input{capitulos/08_quantas_explicacoes_cabem}

% O capítulo 9 — a volta 4: ver mundos basta, ou é preciso mexer no mundo?

\input{capitulos/09_ver_mundos_basta}
...
% O capítulo 10 — a travessia: o vigia de margem não vê a dependência.

\input{capitulos/10_vigia_nao_ve}

% O capítulo 11 — a travessia: o que separa duas explicações que a observação empatou.

\input{capitulos/11_desenho_da_intervencao}

% O capítulo 12 — a travessia: proteger e adaptar disputam o mesmo relógio.

\input{capitulos/12_partilha_do_relogio}

% O capítulo 13 — a travessia: quanto do passado se pode apagar antes de deixar de aprender.

\input{capitulos/13_memoria_que_aprende}

% O capítulo 14 — a travessia: a direção do tempo, com orçamento de alarme.

\input{capitulos/14_vigia_da_direcao}

% O capítulo 15 — a travessia: quem cai primeiro, e se essa ordem se repete.

\input{capitulos/15_quem_cai_primeiro}
...
% O capítulo 16 — andar 1: o que não se compra, começando pelo mundo que não foi observado.

\input{capitulos/16_mundo_que_faltou}

% O capítulo 17 — andar 1: o que foi apagado, e se há preço que traga o passado de volta.

\input{capitulos/17_o_que_foi_apagado}

% O capítulo 18 — andar 1: o que a margem não conta.

\input{capitulos/18_o_que_a_margem_nao_conta}
...
% O capítulo 19 — andar 2: o enquadramento, começando pela série.

\input{capitulos/19_o_pedaco_escolhido}

% O capítulo 20 — andar 2: o agora, e as três bordas do enquadramento.

\input{capitulos/20_o_agora}
...
% O capítulo 21 — o fecho: a pergunta da raiz, refeita com tudo o que foi construído.

\input{capitulos/21_o_fecho}
