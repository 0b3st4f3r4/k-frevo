...
A pergunta de onde tudo começa é a mais simples que existe: quanto se pode perder amanhã. A
resposta usual é uma linha erguida sobre os piores dias do passado, e essa linha tem um preço
que se mede. Sem ela não há promessa, e sem promessa não há entrega para comparar.
...
% O capítulo 3 — volta 1: a forma da mudança decide o preço do silêncio.

\input{capitulos/03_forma_decide_o_preco.tex}

% O capítulo 4 — a segunda forma que a margem não vê: o calendário.

\input{capitulos/04_o_dia_da_semana.tex}

% O capítulo 5 — a última forma da volta 1: a dependência, que não está em margem nenhuma.

\input{capitulos/05_dia_em_que_tudo_cai}

% O capítulo 6 — a volta 2: a estatística que estima não anuncia, e o controle diz por quê.

\input{capitulos/06_estatistica_que_nao_anuncia}

% O capítulo 7 — a volta 3: quantas explicações cabem nos mesmos dados.

\input{capitulos/07_quantas_explicacoes_cabem}

% O capítulo 8 — a volta 4: ver mundos basta, ou é preciso mexer no mundo?

\input{capitulos/08_ver_mundos_basta}
...
% O capítulo 9 — a travessia: o vigia de margem não vê a dependência.

\input{capitulos/09_vigia_nao_ve}

% O capítulo 10 — a travessia: o que separa duas explicações que a observação empatou.

\input{capitulos/10_desenho_da_intervencao}

% O capítulo 11 — a travessia: proteger e adaptar disputam o mesmo relógio.

\input{capitulos/11_partilha_do_relogio}

% O capítulo 12 — a travessia: quanto do passado se pode apagar antes de deixar de aprender.

\input{capitulos/12_memoria_que_aprende}

% O capítulo 13 — a travessia: a direção do tempo, com orçamento de alarme.

\input{capitulos/13_vigia_da_direcao}

% O capítulo 14 — a travessia: quem cai primeiro, e se essa ordem se repete.

\input{capitulos/14_quem_cai_primeiro}
...
% O capítulo 15 — andar 1: o que não se compra, começando pelo mundo que não foi observado.

\input{capitulos/15_mundo_que_faltou}

% O capítulo 16 — andar 1: o que foi apagado, e se há preço que traga o passado de volta.

\input{capitulos/16_o_que_foi_apagado}

% O capítulo 17 — andar 1: o que a margem não conta.

\input{capitulos/17_o_que_a_margem_nao_conta}
...
% O capítulo 18 — andar 2: o enquadramento, começando pela série.

\input{capitulos/18_o_pedaco_escolhido}

% O capítulo 19 — andar 2: o agora, e as três bordas do enquadramento.

\input{capitulos/19_o_agora}
...
% O capítulo 20 — o fecho: a pergunta da raiz, refeita com tudo o que foi construído.

\input{capitulos/20_o_fecho}
