...
O capítulo anterior terminou com uma pergunta que atravessou dois capítulos: apagar os dias apaga, com eles,
...
O instrumento é o mesmo das outras travessias: uma estatística, um limiar e um orçamento declarado\cite{gallavotti1995dynamical,godreche2018characterising,roldan2017statistics}. A
estatística é o terceiro momento do incremento, e a razão de ser dele está na proposição abaixo.
Uma série é \textbf{assimétrica} quando o lado de cima dela difere do lado de baixo: subidas maiores
que as quedas, ou o contrário. O que mede isso é a média do cubo do incremento padronizado, que é
zero num mundo simétrico e cresce quando um dos lados fica mais grosso. O bloco aqui não é o do
capítulo do corte: são duzentos e cinquenta e dois dias que não se sobrepõem --- o mesmo tamanho da janela de lá, e sem
a sobreposição, para que cada bloco conte uma vez na contagem dos alarmes.

\begin{proposicao}[o desvio do nulo]\label{prop:nulo-da-direcao}
Se os incrementos padronizados de um mundo forem normais padrão e independentes, a média do cubo do
incremento padronizado tem média zero e desvio $\sqrt{15/n}$ numa janela de $n$ dias.
\end{proposicao}
...
Com a janela de \numDirecaoJanela{} dias, a \cref{prop:nulo-da-direcao} dá \numDirecaoConta{}, e o limiar de
...
O motivo da diferença não é um erro de conta, e também não é o agrupamento: é a padronização. A conta
supõe que o incremento padronizado de um dia não saiba nada dos outros, e o instrumento padroniza o
bloco inteiro pela média e pelo desvio \emph{estimados} da janela anterior --- de modo que os
\numDirecaoJanela{} dias do bloco compartilham a mesma média, e a média do cubo ganha um termo que a
conta não previu. Num mundo simétrico e de escala constante, sem agrupamento nenhum, é esse
compartilhamento que responde pela diferença. O agrupamento tem o seu lugar, e é adiante: ele explica
a dispersão medida nos blocos do dado real.
...
Duas coisas do desenho pesam na leitura. As linhas do limiar honesto ficam encostadas na linha do
zero, empurradas para lá pelas barras extremas, de modo que, para um ano de tamanho médio, cruzar o
limiar e sair do zero têm quase a mesma altura no papel. E o eixo conta blocos, não anos da série: o
primeiro bloco já deixou \numDirecaoJanela{} dias de história atrás de si, porque a estatística se
padroniza pela janela anterior --- o primeiro ano que o vigia julga não é o primeiro ano do dado.
