% =============================================================================
% CAPÍTULO 6 — volta 1 (AGENTS.md §6): a forma que o relógio desenha.
%
% O fracasso que o abre, com número na mão: levado à carga elétrica diária da Dinamarca no
% mesmo orçamento de um alarme por ano, o vigia do capítulo 4 dispara 4,2 vezes por ano e
...
O domingo consome \numCalendarioDomingoAbaixoPct\% menos que o dia médio, e o padrão se repete em quase todas as semanas dos cinco anos --- as exceções são as de Natal e Ano-Novo, quando o feriado do meio da semana pesa mais que o domingo. Vale medir o tamanho desse perfil
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lr}
    ...
    \hline
  \end{tabular}
  \caption{O tamanho do calendário, medido. Nas cargas europeias a semana desenha em torno de um
    desvio-padrão, e a França é o caso mais fraco; no índice americano ela desenha quase nada.}
  \label{tab:perfil}
\end{table}

A carga dinamarquesa tem \numCalendarioAmplitudeCarga{} desvios-padrão entre o dia mais cheio e
o mais vazio \cite{opsd2019time}, e as praças vizinhas vão de \numCalendarioAmplitudeFr{} a
...
ser erguido dentro da célula: o domingo mede o domingo, a terça mede a terça\cite{tibshirani2019conformal}.
...
O preço aparece na conta do capítulo anterior, lida célula por célula, e ele cabe numa
proposição.
% aterramento: C
A partição tem um número só, e é ele que a proposição usa: $C$ é o número de células em que a
janela é repartida --- sete, quando a partição é a semana, com o domingo numa célula, a segunda
na outra, e assim por diante.

\begin{proposicao}[o preço da partição]\label{prop:particao}
Repartida a janela de $n$ dias em $C$ células iguais, o alarme mais raro que a leitura consegue
declarar soa $C/(n+C)$ das vezes por dia.
\end{proposicao}

\begin{proof}
Cada célula recebe $n/C$ dias, e a conta do corte vale dentro dela: o posto mais fundo que uma
célula declara é o primeiro da fila dela, de modo que ela soa $1/(n/C + 1)$ das vezes por dia, que
é $C/(n+C)$. Cada dia pertence a uma única célula, e a taxa da leitura inteira é a mesma delas.
Com $C$ igual a um a forma devolve $1/(n+1)$, a conta do corte.
\end{proof}

Repartida por $C$ células, a janela de \numCalendarioJanela{} dias deixa de ser memória inteira de
cada decisão: cada célula recebe $n/C$ dias, e sete células tornam o alarme quase sete vezes mais
frequente do que a leitura crua.
...
O silêncio volta, e o preço dele está na tabela: o primeiro dobrar de janela derruba os alarmes por um
...
Vale olhar quanto do desenho é cinza. Os dois dias de fim de semana ocupam quase um terço da largura
do painel, e o consumo ali fica abaixo do dos cinco dias anteriores: o que o vigia cru chama de dia
ruim é um terço dos dias, e volta toda semana no mesmo lugar.
...
\begin{figure}[htbp]
  ...
  \caption{À esquerda, o que a partição cobra: alarmes por ano contra o que a memória declara,
    para as quatro leituras da \cref{tab:celulas}. O eixo dos alarmes é logarítmico, de modo que
    distâncias iguais no papel são multiplicações iguais no preço. À direita, o que a memória compra de
    volta, com a janela crescendo e a partição semanal fixa.}
  \label{fig:particao}
\end{figure}
...
Falta o \textbf{controle}, porque a partição é uma decisão e não uma melhoria. O controle é a
mesma rotina aplicada a um mundo onde não há o que a decisão possa ler. Aqui ele é o índice
acionário americano, que não tem semana: o que a partição faz lá diz o que ela custa quando não
há calendário para ler.

Nas \numSerieDias{} observações do índice, a leitura crua dá \numCalendarioIndiceCruAlarmesAno{}
alarmes por ano contra o piso de \numCalendarioIndiceCruPisoAno{} que a memória do índice
declara --- uma diferença de pouco mais de vinte por cento, da mesma natureza da que o capítulo
anterior discutiu. A leitura por dia da semana,
com a mesma janela, sobe para \numCalendarioIndiceSemanaAlarmesAno{} alarmes por ano, contra os
\numCalendarioIndiceSemanaPisoAno{} que a memória declara. Nada foi
encontrado em troca: gastou-se um fator de quatro em falso alarme para descobrir que segunda
não é domingo no mercado.

A decisão fica então com a \cref{tab:perfil}. Onde a semana desenha em torno de um desvio-padrão,
ler o calendário é obrigatório --- a alternativa é um vigia que relata o relógio; e é a França, com
\numCalendarioAmplitudeFr, que fica na fronteira. Onde desenha \numCalendarioAmplitudeIndice, a partição
é só despesa. E o número que decide é a amplitude do perfil da série, medida.
...
E fica o fracasso seguinte, que a partição tornou visível sem resolver. A partição em
células supõe que se saiba, de antemão, por onde partir os dias --- e o calendário é o caso
fácil, porque a célula está escrita no relógio. Existe uma partição que ninguém escreveu no relógio: duas séries que passam a cair juntas, sem que nenhuma das duas mude de
tamanho. Cada uma continua com exatamente os mesmos números, no mesmo dia da semana, e o
par delas muda tudo o que se pode perder junto. Nenhuma célula da partição vê isso, e
