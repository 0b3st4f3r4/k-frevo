% =============================================================================
% CAPÍTULO 4 — volta 1, a última forma: a dependência que a margem não vê.
%
...
% capítulos 2 e 3 pode ver isso — a informação não está em nenhuma das duas séries.
%
...
calendário. Todos leem uma margem. Este capítulo trata de duas ao mesmo tempo, e começa por uma
...
O par é o índice americano e o brasileiro: \numDependenciaDias{} dias em que os dois
existiram, um quarto de século de mercado. Cada um rompeu o seu próprio corte em torno de
\numDependenciaTaxaAPct\% dos dias --- \numDependenciaRompeA{} rompimentos de um lado,
\numDependenciaRompeB{} do outro ---, que é a promessa do primeiro capítulo sendo cumprida duas
vezes.
...
dos dias, contra os \numDependenciaEsperadoPct\% que a independência previria. Em vezes, são
\numDependenciaExcesso{} vezes mais.
...
quantos dias as duas romperam o próprio corte, contra quantos dias a independência previria.
...
\begin{table}[htbp]
  ...
  \caption{A mesma série, com o par trocado. A coluna da esquerda é a que importa: a
    ...
    série dela não foi tocada. Só o pareamento mudou.}
  \label{tab:par}
\end{table}
...
uma rompe, perde \numDependenciaPerdaUmaPernaPct\%. A razão é de \numDependenciaPerdaRazao{}
vezes. Não é o mesmo dia ruim com duas pernas: é um dia ruim mais fundo, e é justamente o dia
em que as duas proteções são chamadas ao mesmo tempo.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figuras/E05_dependencia_2.pdf}
  \caption{À esquerda de cada barra está o par como o mundo fez; as outras quatro são o mesmo
    par com o segundo mercado deslocado, e a linha tracejada é o que a independência prevê.
...
que as duas coisas rompem, e compare com o que a independência previria}. A razão entre os dois
...
que o número. O vigia do segundo capítulo e a célula do terceiro leem margens, e a dependência
não está em margem nenhuma: mudar o par deixa as duas séries idênticas a si mesmas e muda a
conta. Não é que aqueles instrumentos sejam fracos --- é que a pergunta que este capítulo faz não
é uma pergunta sobre uma série.

O fracasso seguinte nasce do próprio número que ficou. \numDependenciaExcesso{} vezes a
independência é uma média sobre vinte e seis anos; ela diz que os dois caem juntos mais do que a
