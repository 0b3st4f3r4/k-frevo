% =============================================================================
% CAPÍTULO 13 — volta 4 (AGENTS.md §6): ver mundos basta, ou é preciso mexer no mundo?
%
...
O capítulo da geometria deixou uma região: quatro estatísticas medidas num mercado não escolhem
uma explicação entre as duas que sobreviveram, e encolher a região com mais estatística tem
limite, porque todas elas vêm do mesmo pedaço de passado. O material acabou de ser interrogado: a
cauda convive com dias independentes de lei pesada, e sobrou a direção que os extremos cedo
apontam --- uma lei que anda no tempo. A saída que se oferece sozinha é outra: acumular mundos. Mais mercados, mais períodos, mais domínios --- e o que era indistinguível num deles fica
separado pelo conjunto.

A tentativa tem uma forma precisa, e vale escrevê-la: o mecanismo é o mesmo em toda parte, e cada
mercado só tem a sua escala \cite{peters2016causal,scholkopf2021causal,ghassami2017learning}. Se for assim, observar mais mundos identifica o mecanismo, e a
...
A promessa do corte --- a linha no décimo terceiro pior dos últimos
...
para o outro tem a mesma forma nos três.

Se o mecanismo por trás deles também é o mesmo, é outra pergunta, e ela é a da próxima seção.
...
calma e uma agitada, com o estado durando ---, e as quatro estatísticas que ele mostrou --- a taxa de rompimento, o pior bloco de sessenta dias, a
mediana dos blocos e a fração de blocos acima do dobro da promessa. Uma explicação serve a um mercado
quando reproduz as quatro com as tolerâncias declaradas lá.

O primeiro mercado admite \numObservarSobreviventesIndice{} explicações numa varredura --- e entre
\numObservarIndiceMenor{} e \numObservarIndiceMaior{} em \numObservarSementesControle{} sementes,
mediana \numObservarIndiceMediana{}, de modo que o número de uma varredura sozinha não é uma
medida. O segundo admite \numObservarSobreviventesIbov. O terceiro admite
\numObservarSobreviventesBitcoin. Ao todo, \numObservarUniao{} explicações sobrevivem em algum
deles.

Quantas sobrevivem aos três, ao mesmo tempo, é o número que a medição persegue:
\numObservarIntersecao.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{figuras/E08_observar_ou_intervir_1.pdf}
  \caption{Quantas explicações cada mercado admite sozinho, numa varredura, e a linha da interseção.
    Cada barra é uma varredura só: a do mercado com mais dados vai de \numObservarIndiceMenor{} a
    \numObservarIndiceMaior{} em \numObservarSementesControle{} sementes. A linha colada no eixo é
    a interseção --- nenhuma explicação serve aos três ---, e não uma barra que faltou.}
  \label{fig:admite}
\end{figure}
...
diferentes. A pergunta dura é se eles discordam do \textbf{mecanismo}, e para isso a lei passa a ser
...
mercados. A grade fina tem \numObservarTriplosFinos, e \numObservarFina{} deles servem\cite{mey2024invariant}. As duas
...
e uma estatística grosseira não separa nada\cite{pope2005analysis}. A taxa vai longe, porque é a margem --- e a margem
...
Enriquecer exatamente a dimensão que discordava não consertou nada: piorou. E a medição carrega
uma lição de método.
% aterramento: semente
Um mundo sorteado se repete quando se guarda a \textbf{semente}: ela é o número que faz o sorteio
sair o mesmo de novo, e é o que permite a outra pessoa conferir a conta. Com uma semente por
configuração, a mesma configuração deu pior bloco de quinze a dezenove em cinco sementes ---
variação maior que qualquer diferença entre configurações. O veredito acima só
...
\begin{figure}[htbp]
  ...
  \caption{A mediana do pior bloco e a do excesso, por configuração e por mercado, com o valor do
    ...
    No painel do pior bloco as faixas de dez a noventa por cento se sobrepõem, de modo que a
    distância aparente entre barra e estrela é maior do que a distância entre as distribuições.}
...
comum. As marginais dos três mercados concordam --- a promessa do corte é cumprida
três vezes, com a mesma conta e o mesmo número ---, e o agrupamento discorda, com o gargalo
isolado na estatística que mede como os dias ruins se juntam\cite{bing2023invariance}. Com a lei compartilhada o vazio
...
que foi medido: uma família \textbf{mais rica} --- memória longa além de troca de regimes, parâmetros
que variam no tempo \cite{palma2010efficient,dahlhaus2006nonparametric,palma2013estimation} --- ainda não foi testada, e é o único caminho que mantém vivo o refutador
desta pergunta. O que está medido, e é o que o capítulo assina, é mais estreito e ainda assim
forte: na família de mundos que se sabe construir, \textbf{ver mundos não basta}.

E a porta é a pergunta seguinte: atravessá-la com instrumento na mão --- medir o que a família de
parâmetros que variam no tempo faz com as janelas e com as contas que este livro já fechou --- é
o trabalho da última volta.
