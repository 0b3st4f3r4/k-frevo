% =============================================================================
% CAPÍTULO 8 — volta 2 (AGENTS.md §6): estabilidade e a sua quebra, como uma medida só.
%
...
--- um filtro para o nível, um detector para a quebra ---, e o capítulo do orçamento já mostrou que o par de
...
O palco é a mesma série de sempre, com \numEstabilidadeDias{} dias utilizáveis, o corte da \cref{prop:conta} --- o décimo terceiro pior de \numCorteJanela{} --- e o bloco de
\numBlocoDias{} dias do alarme. Para cada dia, quatro sinais olhando para trás, e um alvo olhando
...
Os quatro sinais são o \textbf{movimento do corte} (a estimativa se mexeu), a \textbf{contagem
recente} (o vigia do capítulo do orçamento), o \textbf{nível do corte} (a própria estimativa) e o
\textbf{nível sobre a oscilação}, que é a mesma estimativa dividida pela oscilação de agora --- a
dos \numEstabilidadeOscilacaoBloco{} dias, o mesmo bloco do capítulo do alarme.

% aterramento: posto
A medida é a correlação de posto, e a escolha é deliberada: nada aqui supõe relação linear, e é
justamente na cauda que a relação deixa de ser linear. A correlação de posto olha a ordem e não os
valores: ela vale um quando os maiores de um sinal são sempre os maiores do alvo, menos um quando são
sempre os menores, e zero quando a ordem de um não diz nada sobre a ordem do outro. Não muda quando um
dia isolado fica dez vezes maior, porque isso não muda o lugar dele na fila.
...
% aterramento: décimo
O perfil por décimo reparte os dias em dez fatias iguais, da mais baixa à mais alta do sinal, e
mostra a média do alvo em cada uma: ele dá a forma da relação, que um número só não dá.
% aterramento: cauda
A \textbf{cauda} é o punhado de dias extremos de uma série --- os maiores e os menores ---, e é
onde o prejuízo mora, tanto mais quanto menos dias ela tiver.
...
O nível do corte é parente próximo da oscilação dos \numEstabilidadeOscilacaoBloco{} dias do bloco do
capítulo do alarme --- a correlação entre os dois é
de \numEstabilidadeNivelComOscilacaoPosto, e o sinal é negativo porque corte fundo quer dizer
ano agitado. Com o denominador que a frase pedia antes, a oscilação dos
\numEstabilidadeOscilacaoAno{} dias, o posto do mesmo sinal cai de \numEstabilidadeRelativoPosto{} para
\numEstabilidadeRelativoAnoPosto{}: o que ele carrega além da oscilação é do bloco de sessenta dias, e
não do ano. Sobra a dúvida óbvia: o nível anuncia alguma coisa, ou só repete a volatilidade que
...
igual à do mercado. Três dos quatro sinais foram medidos no mundo que nunca muda, e o resultado é
este.
...
\begin{tabular}{lrr}
    ...
    \hline
  \end{tabular}}
  \caption{Os mesmos sinais nos dois mundos, para os três que o mundo parado tem medidos --- o nível
    sobre a oscilação ficou sem controle. No mundo em que nada muda não há o que anunciar, e ainda
    assim o nível do corte produz correlação.}
  \label{tab:controle}
\end{table}
...
A lição vale além daqui, e é a razão desta medição existir: \textbf{uma medida sem o seu
...
O que a prática vigia --- a linha se mexeu, é hora de recalibrar --- é o único dos quatro sinais
que não vale nada, nem onde há o que prever nem onde não há.

\section{Quantas barras eu ergui}

O capítulo julga cada comparação contra uma barra de dois desvios: são \numMultiplicidadeSinais{}
sinais dentro de \numMultiplicidadeFaixas{} faixas, em cada série --- \numMultiplicidadeComparacoes{}
barras erguidas. A barra está certa para \emph{uma} comparação: nos mundos que não mudam ela deixa
passar \numMultiplicidadeTaxaComparacaoPct{} das vezes, que é o que dois desvios prometem. O que ela
não mede é a bateria\cite{lew2019reckless,wasserstein2016statement}.

A conta da bateria é aritmética e não tem escapatória: com \numMultiplicidadeComparacoes{}
comparações, a chance de que ao menos uma cruze a barra por acaso é de
\numMultiplicidadeFwerExataPct{}\%. Para manter os mesmos cinco por cento no conjunto, a barra teria
de estar em \numMultiplicidadeQuantilFamilia{} desvios.

E não é teoria. Rodando a \emph{mesma} bateria dentro dos mundos que nunca mudam --- o nulo do
próprio capítulo, \numMultiplicidadeMundos{} mundos de \numMultiplicidadeDias{} dias ---, em
\numMultiplicidadeTaxaBateriaPct{}\% deles alguma barra acende: \numMultiplicidadeMundosComAlguma{}
de \numMultiplicidadeMundos{}, com \numMultiplicidadeTravessiasMedia{} travessias por mundo e até
\numMultiplicidadeTravessiasMaxima{} no mesmo mundo. E o medido é \emph{maior} que o previsto ----
\numMultiplicidadeTaxaBateriaPct{} contra \numMultiplicidadeFwerExataPct{} ----, porque as
quarenta comparações não são independentes: os quatro sinais leem os mesmos dias, e as dez faixas
são pedaços ordenados do mesmo perfil. A conta independente é o piso, e não o valor.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figuras/E24_multiplicidade_1.pdf}
  \caption{A barra de uma comparação e a barra da bateria. A curva é a chance de ao menos uma
    travessia por acaso, contra o número de comparações; a linha tracejada marca as
    \numMultiplicidadeComparacoes{} comparações do capítulo, e o ponto quadrado é o que a bateria
    mediu nos mundos que não mudam.}
  \label{fig:bateria}
\end{figure}

Isso não derruba a leitura do capítulo, e vale dizer por quê: o que ele persegue é a \emph{forma}
do perfil por décimos, e uma travessia de barra não é a mesma coisa que um perfil que sobe. O que
esta seção mede é o preço de julgar uma bateria pela barra de uma comparação: num mundo que não
muda, \textbf{quarenta barras de dois desvios acendem alguma coisa em quase todo mundo} --- e é por
isso que a barra da bateria é outra.
...
exige medir o chão em que ele foi medido. Toda vez que um número parecer anunciar
...
não dar para as duas leituras ao mesmo tempo. Antes de medir essa quantidade, porém, a régua
que este capítulo instalou cobra a sua primeira cobrança ilustre: o alerta que o mundo inteiro
usa, e que nunca declarou o par. É ele que vem agora.
