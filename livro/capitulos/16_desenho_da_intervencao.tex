...
O capítulo dos mundos que não bastam deixou duas famílias de explicação de pé para a mesma série. Numa, a oscilação de amanhã
...
com o que comparar. Feito duas vezes, na contenção de \numDesenhoApertadoContensao{} dias que a
\cref{tab:resposta} mede, esse experimento separa as explicações em \numDesenhoSeparaPoucasPct\%
das repetições: quem decide assim está decidindo no sorteio, e vai decidir com ele.

Vale dizer onde o empate é apertado, porque é ali que a observação gastou tudo o que tinha. Contra
as quatro leituras do capítulo das explicações que cabem, os dois mundos concordam dentro da
tolerância declarada, e
...
\emph{resposta} do mundo a essa contenção é a média de $|x|$ --- o tamanho do afastamento da série em
cada dia --- nos $m$ dias seguintes à soltura,
dividida pela média de $|x|$ nos $m$ dias anteriores à contenção. O número $m$ é a janela de
espera: quantos dias depois da soltura ainda se olha. Uma \emph{replicata} é um mundo
sorteado inteiro, segurado e observado uma vez --- o mesmo experimento inteiro, repetido do
zero, que é o que dá dispersão para medir.
\end{definicao}
...
Vale ler a definição com números pequenos. Com $k$ de cinco dias e $m$ de dez, a resposta é a média
de $|x|$ nos dez dias seguintes à soltura dividida pela média de $|x|$ nos dez dias anteriores à
contenção: uma razão de um é um mundo que voltou ao que era, e uma razão de meio é um mundo que
voltou com metade da oscilação.

% aterramento: memoria
O que separa as duas explicações é a forma da resposta.
Dizer que a \textbf{memória} mora na série é
dizer que o tamanho de hoje depende do tamanho de ontem: o mundo carrega o próprio passado, e
carrega menos dele a cada dia que passa. Onde a memória mora na série, segurar apaga
...
% aterramento: estacionaria
Um mundo em que a lei não muda com o dia chama-se \textbf{estacionário}: cada dia sorteia da mesma
lei, e o que muda de um dia para o outro é só o que o sorteio deu. É o mundo de que os capítulos
anteriores partem sempre que dizem «a mesma lei», e a mudança estudada aqui é justamente a lei
deixando de ser a mesma.
% aterramento: var
% aterramento: x
% aterramento: omega
% aterramento: alpha
% aterramento: beta
% aterramento: ponto-fixo
A lei que o mundo do experimento obedece tem quatro peças, e as quatro se leem em uma linha.
$x(t)$ é o valor da série no dia $t$; $v(t)$ é a variância declarada daquele dia, o quadrado do
tamanho típico que a barra da \cref{prop:barra} mediu. A oscilação de hoje é uma mistura de três
coisas: um chão que não depende de nada, $\omega$; o que o mundo andou ontem, com peso $\alpha$;
e a própria oscilação de ontem, com peso $\beta$. O peso da memória é $\beta$, e é ele que faz a
oscilação de hoje parecer com a de ontem. Uma recursão dessas caminha para um número em que ela
não se mexe mais --- o \textbf{ponto fixo}, o valor que posto do lado direito devolve ele mesmo.

\begin{proposicao}[o piso da contenção]\label{prop:piso-contencao}
Num mundo cuja oscilação obedece $v(t) = \omega + \alpha\, x(t-1)^2 + \beta\, v(t-1)$
\cite{bollerslev1986generalized}, com $\alpha > 0$, $0 < \beta < 1$ e $\alpha+\beta < 1$, a variância no $k$-ésimo dia de uma contenção é
\[
...
\begin{proof}
Da contenção em diante a série é zero, de modo que a partir do segundo dia $x(t-1)^2 = 0$ e a
recursão perde o termo da observação: $v(t) = \omega + \beta\, v(t-1)$. No primeiro dia o termo
ainda é o do último dia antes dela, e é por isso que a forma fechada toma a variância daquele
primeiro dia como $v(0)$. Iterando $k$ vezes a partir dele chega-se à forma fechada, e o limite quando $k$ cresce é o ponto fixo da recursão, $\omega/(1-\beta)$. A
...
\section{A volta é um produto}

A proposição anterior assina a metade contida da volta: com passo determinístico o ganho de cada
dia é exatamente $\beta$, e a profundidade se compra em $\beta^k$. Falta a outra metade, e ela não
é um detalhe --- o passo de cada dia \emph{sorteia}. A volta não é o ganho de um dia elevado a um
expoente: ela é o \emph{produto} dos ganhos que cada dia trouxe.

% aterramento: ganho-do-dia
O \textbf{ganho do dia} é a parte da oscilação de hoje que passa para amanhã, e ele tem a forma
$\beta + \alpha z^2$, com $z$ o sorteio daquele dia. Ganho abaixo de um apaga o que recebeu;
acima de um, amplifica. Com $\alpha$ nulo o ganho é a constante da proposição, e é por isso que a
proposição é o caso em que a profundidade assinada é exata.

% aterramento: influencia
A \textbf{influência do dia zero} --- quanto de um dia qualquer sobrevive ao caminho até hoje --- é
o produto acumulado desses ganhos. É ela que a memória longa mede, e é ela que a proposição
assinava no caso determinístico. Com ganhos que sorteiam, duas contas da mesma soltura aparecem, e
elas não coincidem.

% aterramento: expoente-do-produto
% aterramento: conta-da-media
A primeira é o \textbf{expoente do produto}: a média dos logaritmos dos ganhos, que é o ritmo da
\emph{mediana} da influência entre mundos. A segunda é a \textbf{conta da média}: o logaritmo da
média do ganho, $\log(\alpha + \beta)$, que é o ritmo da \emph{média} da influência. O logaritmo é
côncavo, de modo que o expoente fica abaixo da conta sempre que o ganho sorteia --- e a distância
entre os dois é o preço de ler o produto pela média. Na calibração deste capítulo o expoente medido
é \numProdutoExpoenteCapitulo{} contra a conta de \numProdutoContaCapitulo{}, um vão de
\numProdutoVaoCapitulo; no contraste de mesmo envelope o vão é de \numProdutoVaoContraste, sete
vezes maior. O vão existe nas duas, e o que a calibração decide é o tamanho dele.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figuras/E49_produto_1.pdf}
  \caption{A influência do dia zero contra o horizonte, nas duas calibrações. Em cada painel, a
    mediana dos mundos desce abaixo da média, e as duas descem acima da linha pontilhada do ganho
    mínimo. No painel da direita as duas curvas se abrem.}
  \label{fig:produto}
\end{figure}

% aterramento: teorema-ergodico-multiplicativo
O que a média esconde tem nome. Com passos que sorteiam, o ritmo do produto existe e é a média dos
log-ganhos --- não o logaritmo da média. É o \textbf{teorema ergódico multiplicativo},
e é ele que dá ao produto um expoente \cite{oseledets1968multiplicative}. É por isso que a
mediana da influência desce no expoente medido e não na conta da média: a segunda descreve a média
de muitos mundos, e a primeira descreve o mundo típico.

E a composição pode sair pior do que qualquer fator anunciava. \numProdutoCresceCapituloPct\% dos
mundos têm uma influência que \emph{cresce} antes de cair na calibração deste capítulo --- e
\numProdutoCresceContrastePct\% no contraste ---, e \numProdutoJanelasAcimaCapituloPct\% das
janelas de espera entregam um produto acima da conta da média. A tradução é declarada, porque a
fonte é mais geral do que o caso medido: o resultado é de matrizes, e o ganho de um dia é o caso de
um número só \cite{botha1998products}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figuras/E49_produto_2.pdf}
  \caption{Os desviantes. À esquerda, a fração de mundos cuja influência cresce antes de cair; à
    direita, a fração de janelas de espera cujo produto bate a conta da média. No contraste as duas
    frações são maiores do que na calibração do capítulo --- menos a última.}
  \label{fig:desviantes}
\end{figure}

Isso muda a leitura do que o capítulo mede. A cauda que mente para a álgebra, medida adiante, não é
um acidente do mundo escondido: ela mora também aqui, no produto de ganhos inofensivos, e é o
mecanismo que a resposta carrega. O desenho mede a resposta; esta seção diz de que número ela é
feita.
...
contenções. A
diferença medida sai de quase nada e chega a \numDesenhoDiferencaMaior{} em módulo na contenção de
...
explicações respondem quase igual, e o que a figura mostra são os dois intervalos de confiança
cruzados: o empate está ali, e não em dois pontos coincidentes.
...
Instrumento quieto não é instrumento preciso, e o desenho diz isso antes da conta. A curva do estado
escondido anda colada na linha pontilhada e passa por retrato de estabilidade; a maior barra de erro
do quadro é a dela, no extremo direito. A quietude é do traço, e o que a contenção longa entrega é
uma média mais dispersa.
...
entre replicatas, e as duas respostas precisam ficar separadas.
% aterramento: intervalo-de-confianca
% aterramento: s
% aterramento: z
% aterramento: d
Um \textbf{intervalo de confiança} é a barra da média escrita como um trecho: a média, mais e menos
um número de barras dela, e a confiança declarada diz quantas barras são. Com noventa e cinco por
cento de confiança esse número é quase duas barras, e ele entra na conta escrito $z$. O desvio
medido entre replicatas entra escrito $s$, e a diferença que se insiste em ver entra escrita $d$.
Se a soma dos dois raios tem de caber dentro dessa diferença, o número de replicatas sai de
\[
n \;=\; \Big( \frac{z\, s}{d} \Big)^{2},
\]
e o preço em dias de experimento é $n$ vezes a duração de uma replicata.
...
mundo. Para uma diferença de vinte centésimos do nível do mundo, a conta pede
\numDesenhoVinteCentesimosReplicatas{} replicatas --- um número, que o experimento arredonda para cima
antes de rodar. O melhor desenho é a contenção de \numDesenhoVinteCentesimosContensao{} dias, ao
...
\numDesenhoApertadoContensao{} dias da \cref{tab:resposta} que ganha: a diferença daquela linha,
...
com poucas replicatas, e a conta não sabe disso \cite{taleb2020much}.
...
cumprir, e conferir a promessa por repetição, porque a cauda gorda mente para a álgebra\cite{taleb2020statistical}.
...
o que o dado não tem não se compra com confiança\cite{lei2021conformal}. A travessia decide entre explicações onde há uma
